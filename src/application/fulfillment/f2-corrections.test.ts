...
describe('F2 review corrections', () => {
  it('does not allow Supplier to create Shipment', () => {
    const f = fixture(); const x = preparePacked(f);
    expect(() => f.service.createShipment(x.id, 'SUPPLIER:one', 'shipment')).toThrow();
    expect(f.db.prepare('SELECT COUNT(*) c FROM shipments WHERE fulfillment_id=?').get(x.id)).toEqual({ c: 0 }); f.db.close();
  });
  it('synchronizes Shipment SHIPPED with Fulfillment SHIPPED atomically', () => {
    const f = fixture(); const x = preparePacked(f); f.service.createShipment(x.id, 'YUNELA:operator', 'shipment');
    const s = f.service.transitionShipment(x.id, 'SHIPPED', 'YUNELA:operator', 'ship');
    expect(s.status).toBe('SHIPPED'); expect(f.service.getFulfillment(x.id).status).toBe('SHIPPED'); f.db.close();
  });
  it('does not allow Shipment DELIVERED while Fulfillment is not DELIVERED', () => {
    ...
    expect(f.service.getFulfillment(x.id).status).toBe('PACKED'); f.db.close();
  });
  it('synchronizes LOST Shipment to Fulfillment LOST', () => {
    const f = fixture(); const x = preparePacked(f); f.service.createShipment(x.id, 'YUNELA:operator', 'shipment'); f.service.transitionShipment(x.id, 'SHIPPED', 'YUNELA:operator', 'ship');
    const s = f.service.transitionShipment(x.id, 'LOST', 'YUNELA:operator', 'lost');
    expect(s.status).toBe('LOST'); expect(f.service.getFulfillment(x.id).status).toBe('LOST'); f.db.close();
  });
  it('synchronizes RETURNED Shipment to Fulfillment RETURNED', () => {
    const f = fixture(); const x = preparePacked(f); f.service.createShipment(x.id, 'YUNELA:operator', 'shipment'); f.service.transitionShipment(x.id, 'SHIPPED', 'YUNELA:operator', 'ship'); f.service.transitionShipment(x.id, 'DELIVERED', 'YUNELA:operator', 'deliver');
    const s = f.service.transitionShipment(x.id, 'RETURNED', 'YUNELA:operator', 'return');
    expect(s.status).toBe('RETURNED'); expect(f.service.getFulfillment(x.id).status).toBe('RETURNED'); f.db.close();
  });
  it('rejects approval when replacement tea is not the outstanding operational replacement', () => {
    const f = boxFixture(); const x = f.service.createFulfillmentFromPurchase(f.purchaseId);
    ...
    expect(() => f.service.recordCustomerApproval(x.id, { originalTeaId:f.teaId, originalLotId:f.lotId, replacementTeaId:f.teaId, reason:'bad', actor:'YUNELA:operator', customerDecision:'APPROVED', timestamp:new Date().toISOString() }, 'approval-bad')).toThrow(); f.db.close();
  });
  it('rejects approval when replacement lot does not belong to replacement tea', () => {
    const f = boxFixture(); const x = f.service.createFulfillmentFromPurchase(f.purchaseId);
    f.service.handleDiscoveryBoxShortage(x.id, { originalTeaId:f.teaId, originalLotId:f.lotId, proposedReplacementTeaId:f.replacementTeaId, proposedReplacementLotId:f.replacementLotId, reason:'shortage' }, 'YUNELA:operator', 'shortage');
    expect(() => f.service.recordCustomerApproval(x.id, { originalTeaId:f.teaId, originalLotId:f.lotId, replacementTeaId:f.replacementTeaId, replacementLotId:f.lotId, reason:'bad lot', actor:'YUNELA:operator', customerDecision:'APPROVED', timestamp:new Date().toISOString() }, 'approval-bad-lot')).toThrow(); f.db.close();
  });
  it('rejects approval without an outstanding shortage', () => {
    const f = boxFixture(); const x = f.service.createFulfillmentFromPurchase(f.purchaseId);
    expect(() => f.service.recordCustomerApproval(x.id, { originalTeaId:f.teaId, replacementTeaId:f.replacementTeaId, reason:'arbitrary', actor:'YUNELA:operator', customerDecision:'APPROVED', timestamp:new Date().toISOString() }, 'approval')).toThrow(); f.db.close();
  });
  it('keeps Fulfillment blocked after REJECTED approval', () => {
    ...
    expect(f.service.getFulfillment(x.id).status).toBe('PENDING_CUSTOMER_APPROVAL'); expect(() => f.service.markReady(x.id, 'SYSTEM', 'ready-after-reject')).toThrow(); f.db.close();
  });
  it('accepts valid approval, then allows readiness, without changing OrderItem snapshot', () => {
    const f = boxFixture(); const before=(f.db.prepare('SELECT discovery_box_snapshot_json v FROM order_items WHERE id=?').get(f.itemId) as {v:string}).v; const x=f.service.createFulfillmentFromPurchase(f.purchaseId);
    f.service.handleDiscoveryBoxShortage(x.id,{originalTeaId:f.teaId,originalLotId:f.lotId,proposedReplacementTeaId:f.replacementTeaId,proposedReplacementLotId:f.replacementLotId,reason:'shortage'},'YUNELA:operator','shortage');
    ...
    expect(f.service.markReady(x.id,'SYSTEM','ready-after-approval').status).toBe('READY'); const after=(f.db.prepare('SELECT discovery_box_snapshot_json v FROM order_items WHERE id=?').get(f.itemId) as {v:string}).v; expect(after).toBe(before); f.db.close();
  });
  it('makes duplicate approval idempotent and records one decision event', () => {
    const f=boxFixture(); const x=f.service.createFulfillmentFromPurchase(f.purchaseId); f.service.handleDiscoveryBoxShortage(x.id,{originalTeaId:f.teaId,originalLotId:f.lotId,proposedReplacementTeaId:f.replacementTeaId,proposedReplacementLotId:f.replacementLotId,reason:'shortage'},'YUNELA:operator','shortage');
    const approval={originalTeaId:f.teaId,originalLotId:f.lotId,replacementTeaId:f.replacementTeaId,replacementLotId:f.replacementLotId,reason:'approved',actor:'YUNELA:operator',customerDecision:'APPROVED' as const,timestamp:new Date().toISOString()}; f.service.recordCustomerApproval(x.id,approval,'approve'); f.service.recordCustomerApproval(x.id,approval,'approve-duplicate');
    expect(f.db.prepare("SELECT COUNT(*) c FROM fulfillment_events WHERE fulfillment_id=? AND event_type LIKE '%DISCOVERY_BOX_REPLACEMENT_DECISION%'").get(x.id)).toEqual({c:1}); f.db.close();
  });
  it('returns deterministic success for duplicate audit event append', () => {
    const f=fixture(); const x=f.service.createFulfillmentFromPurchase(f.purchaseId); const repo=new SqliteFulfillmentMutationRepository(f.db);
    const event={id:randomUUID(),fulfillmentId:x.id,eventType:'TEST',fromStatus:undefined,toStatus:undefined,actor:'SYSTEM',occurredAt:new Date().toISOString(),eventKey:`duplicate:${randomUUID()}`,createdAt:new Date().toISOString()};
    expect(repo.appendFulfillmentEvent(event)).toBe(true); expect(repo.appendFulfillmentEvent({...event,id:randomUUID()})).toBe(true); expect(f.db.prepare('SELECT COUNT(*) c FROM fulfillment_events WHERE event_key=?').get(event.eventKey)).toEqual({c:1}); f.db.close();
  });
});
