...
describe('F5 Shipment application',()=>{
  ...
  it('rejects invalid tracking data',()=>{const f=fixture();const p=packed(f);f.service.createShipment(p.id,'OPERATOR:f5','create');expect(()=>f.service.updateShipmentTracking(p.id,'OPERATOR:f5','bad',{trackingUrl:'ftp://bad'})).toThrow(/valid HTTP/);});
