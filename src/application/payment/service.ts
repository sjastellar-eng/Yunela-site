...
export class PaymentService {
  ...
  async createPaymentAttempt(customerId: string, orderId: string, idempotencyKey: string): Promise<PaymentAttempt> {
    this.assertCustomer(customerId); if (!idempotencyKey.trim() || idempotencyKey.length > 128) throw new ApplicationError('VALIDATION_ERROR', 'Idempotency-Key is required and must be at most 128 characters');
    const order = this.getOwnedOrder(customerId, orderId); if (order.status !== 'created') throw new ApplicationError('CONFLICT', 'Only CREATED orders can start a payment attempt');
    const existing = this.attempts.getByOrderAndIdempotency(orderId, idempotencyKey); if (existing) return existing;
    const now = new Date().toISOString(); const attempt: PaymentAttempt = { id: randomUUID(), orderId, customerId, provider: 'MONO', merchantReference: randomUUID(), requestedAmount: createMoney(order.pricingSnapshot.total.amount, UAH), requestedCurrency: UAH, state: 'CREATED', idempotencyKey, createdAt: now, updatedAt: now };
  ...
  async handleMonoWebhook(rawBody: Buffer, signature: string): Promise<{ status: 'accepted' | 'ignored' }> {
    ...
    let event: MonoInvoiceEvent; try { event = JSON.parse(rawBody.toString('utf8')) as MonoInvoiceEvent; } catch { throw new ApplicationError('VALIDATION_ERROR', 'Malformed Mono webhook JSON'); }
    this.validateEventShape(event); const attempt = this.attempts.getByProviderInvoiceId(event.invoiceId); if (!attempt) throw new ApplicationError('NOT_FOUND', 'Unknown Mono invoice');
    if (event.reference !== attempt.merchantReference) throw new ApplicationError('PURCHASE_NOT_AUTHORIZED', 'Mono reference mismatch'); if (event.amount !== attempt.requestedAmount.amount || event.ccy !== MONO_CCY) throw new ApplicationError('PURCHASE_NOT_AUTHORIZED', 'Mono amount or currency mismatch');
