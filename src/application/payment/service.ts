...
import type { OrderRepository } from '../commerce/repositories';
...
export interface PaymentAttemptView extends PaymentAttempt { orderId: string; }
...
export class PaymentService {
  constructor(
    ...
    private readonly mono: MonoPaymentAdapter,
    private readonly analytics: AnalyticsTracker,
    private readonly webhookUrl: string,
    private readonly redirectUrl?: string,
  ) {}

  createPaymentAttempt(customerId: string, orderId: string, idempotencyKey: string): PaymentAttempt {
    ...
    const attempt: PaymentAttempt = {
      id: randomUUID(), orderId, customerId, provider: 'MONO', merchantReference: randomUUID(),
      requestedAmount: createMoney(order.pricingSnapshot.total.amount, UAH), requestedCurrency: UAH,
      state: 'CREATED', idempotencyKey, createdAt: now, updatedAt: now,
    };
    this.attempts.create(attempt);
    void this.initializeInvoice(attempt);
    return attempt;
  }

  private async initializeInvoice(attempt: PaymentAttempt): Promise<void> {
    try {
      ...
      const now = new Date().toISOString();
      this.attempts.save({ ...current, providerInvoiceId: invoice.providerInvoiceId, paymentPageUrl: invoice.paymentPageUrl, state: 'REQUIRES_ACTION', updatedAt: now });
    } catch {
      const current = this.attempts.getById(attempt.id);
  ...
  getPaymentAttempt(customerId: string, id: string): PaymentAttempt {
    this.assertCustomer(customerId);
    const attempt = this.attempts.getById(id);
    if (!attempt) throw new ApplicationError('NOT_FOUND', `Payment attempt ${id} was not found`);
    if (attempt.customerId !== customerId) throw new ApplicationError('PURCHASE_NOT_AUTHORIZED', 'Payment attempt is not owned by the customer');
    return attempt;
  }

  async handleMonoWebhook(rawBody: Buffer, signature: string): Promise<{ status: 'accepted' | 'ignored' }> {
    const validSignature = await this.mono.verifyWebhookSignature(rawBody, signature);
    if (!validSignature) throw new ApplicationError('PURCHASE_NOT_AUTHORIZED', 'Invalid Mono webhook signature');
    ...
    if (attempt.state === 'SUCCEEDED') return { status: 'ignored' };
    if (TERMINAL.has(attempt.state)) return { status: 'ignored' };
    ...
    if (event.status === 'failure') {
      this.attempts.recordTerminalOrProviderState({ id: attempt.id, state: 'DECLINED', providerStatus: event.status, providerModifiedAt: modifiedAt, eventFingerprint: fingerprint, providerReference: event.reference });
      return { status: 'accepted' };
    }
    if (event.status !== 'success') {
      const state: PaymentAttemptState = event.status === 'created' || event.status === 'processing' ? 'REQUIRES_ACTION' : 'INITIATED';
      this.attempts.recordTerminalOrProviderState({ id: attempt.id, state, providerStatus: event.status, providerModifiedAt: modifiedAt, eventFingerprint: fingerprint, providerReference: event.reference });
      return { status: 'accepted' };
    }
    if (this.attempts.getByProviderInvoiceId(event.invoiceId)?.state === 'SUCCEEDED') return { status: 'ignored' };
    const purchase: Purchase = {
      id: randomUUID(), orderId: order.id, customerId: order.customerId,
      amount: createMoney(order.pricingSnapshot.total.amount, UAH), confirmedAt: modifiedAt,
      paymentAttemptId: attempt.id, provider: 'MONO', providerInvoiceId: attempt.providerInvoiceId!, providerReference: event.reference!,
    };
  ...
  private getOwnedOrder(customerId: string, orderId: string): Order {
  ...
  private validateEventShape(event: MonoInvoiceEvent): void {
    if (!event.invoiceId || !event.status || !Number.isInteger(event.amount) || !Number.isInteger(event.ccy) || !event.modifiedDate || !event.reference) throw new ApplicationError('VALIDATION_ERROR', 'Mono webhook is missing required fields');
    if (event.ccy !== MONO_CCY) throw new ApplicationError('PURCHASE_NOT_AUTHORIZED', 'Mono currency must be UAH / ccy 980');
  }
}
