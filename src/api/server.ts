...
async function readJson(request: IncomingMessage): Promise<unknown> { const contentType = request.headers['content-type'] ?? ''; if (!contentType.toLowerCase().startsWith('application/json')) throw new ApplicationError('VALIDATION_ERROR', 'Content-Type must be application/json'); return new Promise((resolve, reject) => { let total = 0; const chunks: Buffer[] = []; let tooLarge = false; request.on('data', (chunk: Buffer | string) => { if (tooLarge) return; const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk); total += buffer.length; if (total > MAX_BODY_BYTES) { tooLarge = true; reject(new ApplicationError('VALIDATION_ERROR', 'Request body is too large')); request.resume(); return; } chunks.push(buffer); }); request.on('end', () => { if (tooLarge) return; try { const text = Buffer.concat(chunks).toString('utf8'); if (!text.trim()) throw new ApplicationError('VALIDATION_ERROR', 'Request body is required'); resolve(JSON.parse(text)); } catch (error) { reject(error instanceof SyntaxError ? new ApplicationError('VALIDATION_ERROR', 'Malformed JSON') : error); } }); request.on('error', reject); }); }
...
async function route(request: IncomingMessage, response: ServerResponse, dependencies: ApiDependencies, id: string): Promise<void> {
  ...
  const discoveryBoxMatch = path.match(/^\/discovery-boxes\/([^/]+)$/); if (discoveryBoxMatch) { const boxId = validatePathId(discoveryBoxMatch[1], 'discoveryBoxId'); if (request.method === 'GET') return sendJson(response, 200, dependencies.discoveryBoxService.getBox(boxId), id); return methodNotAllowed(response, id, 'GET'); }
